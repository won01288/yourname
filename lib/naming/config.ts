...
//
// 결정: 의미가 명확히 하나의 오행에 대응하는 부수만 배속한다 (木火土金水 그 자체와,
// 식물/불/흙/쇠/물이라는 본뜻이 논쟁의 여지가 없는 극소수 확장). 王/玉(금 vs 토), 酉(수 vs 금)처럼
// 출처마다 배속이 갈리는 부수는 넣지 않았다 — 틀린 배속을 넣는 것보다 비워두는(NULL) 쪽이
// CLAUDE.md 2.1의 "환각 금지" 원칙에 맞다. 214부수 중 19개만 채워진 부분표이며,
// 나머지는 hanja.element = NULL로 남는다. 근거: 부수 자체(木/火/土/金/水)와, 자원오행을
// 다루는 다수 작명 자료가 공통으로 드는 확장 사례(불 - 灬/日/赤, 물 - 氵/雨/冫/巛 등).
// 표를 더 채우려면 출처를 특정해 이 표에 추가하고 CLAUDE.md 6장(미결정 사항)에서 갱신한다.
