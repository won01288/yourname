...
// Phase 16(CLAUDE.md 0.6, 2026.8.6) — "같은 사주로 더 추천받기" 추가 라운드에서는 summary·sajuStory를
// 생성하지 않는다(isAdditionalRound=true). 이미 최초 라운드에서 한 번 보여준 사주 해설을 매 라운드
// 반복 생성하는 것은 LLM 비용 낭비이고(2.3의 "이미 확정된 사실만 서술" 범위에도 새로 추가되는 게
// 없다), 화면도 추가된 후보 설명만 필요하다는 사용자 요청에 따른 것이다. candidates[].explanation도
// sajuStory를 전제하지 않는 독립형 버전으로 작성하도록 스키마·프롬프트 모두 분기한다.
...
export interface NamingReport {
  /** "같은 사주로 더 추천받기"(CLAUDE.md 0.6) 추가 라운드에는 없다 — 이미 최초 라운드에서 한 번
   * 보여준 요약을 매 라운드 다시 만들지 않는다(LLM 비용 최소화). */
  summary?: string;
  /** 위와 같은 이유로 추가 라운드에는 없다. */
  sajuStory?: {
    title: string;
    body: string;
  };
...
export interface ExplainCandidatesInput {
  ...
  /** CLAUDE.md 0.6 — "같은 사주로 더 추천받기" 추가 라운드면 true. summary·sajuStory를 생성하지
   * 않고(스키마에서 아예 제외), candidates[].explanation도 sajuStory를 참조하지 않는 독립형
   * 구조로 작성하게 한다. 생략(기본 false)이면 기존과 동일하게 전부 생성한다. */
  isAdditionalRound?: boolean;
}

const HANJA_GLOSSES_SCHEMA = {
  type: "array",
...
// candidates[].explanation 스키마 설명 — hasStory(=isAdditionalRound가 아님)에 따라 sajuStory를
// 전제하는 문구(①/⑤ 단계, 소제목 안내)만 달라지고 나머지 구조(②~④)는 동일하다.
function buildExplanationDescription(hasStory: boolean): string {
  const subtitleClause = hasStory
    ? "이 소제목들은 sajuStory와 달리 번호를 붙여도 된다"
    : "이 소제목들은 번호를 붙여도 된다";
  const step1 = hasStory
    ? "① 사주 결핍 요약 — sajuStory에서 나온 결핍(부족한 오행)을 한 번 더 짧게 환기해 다리를 놓는다(반복이 아니라 이 후보로 넘어가는 도입부)."
    : "① 사주 결핍 요약 — 위 [오행 분포]·[신강/신약 및 용신]에 나온 이 사주의 부족한 오행을 짧게 환기해, 이 후보로 넘어가는 도입부를 놓는다.";
  const step5 = hasStory
    ? "⑤ 종합 마무리 — 위 네 가지(사주·소리·글자·숫자)가 어떻게 하나의 방향으로 수렴하는지 정리하고, sajuStory에서 쓴 것과 같은 비유 세계관을 다시 불러와 연결지어 맺는다."
    : "⑤ 종합 마무리 — 위 네 가지(사주·소리·글자·숫자)가 어떻게 하나의 방향으로 수렴하는지 정리하며 맺는다.";

  return (
    "이 후보를 추천하는 이유를 다음 5단계 구조로, 기존 5~7문장 요약형보다 훨씬 길고 " +
    "촘촘하게(단계마다 최소 한 문단 이상) 풀어 쓴다. 각 단계는 '**소제목**' 형태로 단독 줄에 쓴 " +
    `소제목으로 시작하며(빈 줄로 앞뒤 문단과 구분), ${subtitleClause}` +
    `(예: '① 사주의 결핍을 다시 보면'). ${step1} ` +
    ...
    `좋다는 것이 실질적으로 어떤 의미인지 비유로 풀어 쓴다. ${step5} ` +
    "인명용 한자·불용문자 필터를 통과했다는 사실은 ③ 안에서 자연스럽게 한 번 언급한다. 전문 용어는 " +
    "그대로 쓰되 매번 쉬운 풀이나 비유를 곁들인다. 위에 주어진 근거만 언급하고 새 판정을 만들지 않는다."
  );
}

function buildCandidatesArraySchema(hasStory: boolean) {
  return {
    type: "array",
    items: {
      type: "object",
      properties: {
        hangul: { type: "string" },
        explanation: { type: "string", description: buildExplanationDescription(hasStory) },
        hanjaGlosses: HANJA_GLOSSES_SCHEMA,
...
// CLAUDE.md 0.6 — hasStory가 false면 summary·sajuStory 속성 자체를 스키마에서 뺀다(선택적 필드로
// 두지 않음). json_schema 출력 강제 특성상, 속성이 스키마에 없으면 모델이 그 내용을 아예 생성하지
// 않아 실제 출력 토큰(=비용)이 줄어든다 — 단순히 화면에서 숨기는 것과는 다르다.
function buildResponseSchema(hasStory: boolean) {
  const candidates = buildCandidatesArraySchema(hasStory);
...
// 두 모드가 공유하는 "이미 계산된 사실" 블록 — [사주]부터 [이름 후보]까지.
function buildFactsBlock(input: ExplainCandidatesInput): string {
...
// 두 모드가 공유하는 공통 표기 규칙 — 오행 한자(음) 병기, 전문용어 순화, 소제목 표기.
function buildCommonInstructions(): string {
  return `표기 규칙: summary·sajuStory·explanation 어디서든 오행(木火土金水)을 언급할 때는 한자만 단독으로 쓰지 말고
...
function buildSajuStoryInstructions(): string {
  return `sajuStory 작성 지침: 위 [사주]·[만세력 상세]·[오행 분포]·[신강/신약 및 용신]에 있는 사실만 재료로 삼아, 리포트가
...
// hasStory에 따라 ①/⑤ 단계 문구만 달라진다(sajuStory 참조 여부) — buildExplanationDescription과
// 같은 분기를 자유 서술 지침에서도 반복해 강조한다(기존 프롬프트가 스키마 설명과 자유 서술 지침을
// 이중으로 반복해 강제하던 방식 그대로 유지).
function buildExplanationInstructions(hasStory: boolean): string {
  const step1 = hasStory
    ? `① 사주 결핍 요약: sajuStory에서 나온 결핍(부족한 오행)을 한 번 더 짧게 환기해 이 후보로 넘어가는 다리를
   놓아라(반복이 아니라 도입부 역할).`
    : `① 사주 결핍 요약: 위 [오행 분포]·[신강/신약 및 용신]에 나온 이 사주의 부족한 오행을 짧게 환기해, 이 후보로
   넘어가는 도입부를 놓아라.`;
  const step5 = hasStory
    ? `⑤ 종합 마무리: 위 네 가지(사주·소리·글자·숫자)가 어떻게 하나의 방향으로 수렴하는지 한 문단으로 정리하고,
   sajuStory에서 쓴 것과 같은 비유 세계관을 다시 불러와 연결지어 맺어라.`
    : `⑤ 종합 마무리: 위 네 가지(사주·소리·글자·숫자)가 어떻게 하나의 방향으로 수렴하는지 한 문단으로 정리하며
   맺어라.`;

  return `각 후보 explanation 작성 지침: 이 이름이 선택되기까지 실제로 거친 판단 과정을 딱딱한 나열이 아니라 하나의
...
function buildHanjaGlossesInstructions(): string {
  return `hanjaGlosses 작성 지침: 각 후보 한자 옆에 제공된 영어 뜻풀이(Unihan 사전 뜻풀이)를 근거로, 그 글자를 옥편에서
찾았을 때 나올 법한 한 단어 훈(hun)과, 사용자가 읽을 한글 뜻풀이 한 문장(meaningKo)을 써라. 번역이지 새로운
뜻을 창작하는 것이 아니다 — 주어진 영어 뜻풀이의 범위를 벗어나지 마라.`;
}

function buildStrengthsInstructions(): string {
  return `strengths 작성 지침: 이 후보만의 강점을 짧은 구절 정확히 3개로 써라(각 6~18자 내외, 완결된 문장이 아니어도
...
function buildUserPrompt(input: ExplainCandidatesInput, hasStory: boolean): string {
  const sections = [
    buildFactsBlock(input),
    buildCommonInstructions(),
    ...(hasStory ? [buildSajuStoryInstructions()] : []),
    buildExplanationInstructions(hasStory),
    buildHanjaGlossesInstructions(),
    buildStrengthsInstructions(),
  ];
  return sections.join("\n\n");
}

function buildSystemPrompt(hasStory: boolean): string {
  const storyClause = hasStory
    ? "이 사람의 사주를 은유적이고 감성적인 톤으로 풀어 쓴 sajuStory를 작성한다. " +
      "sajuStory와 각 후보 explanation은 리포트 나열이 아니라 짧은 에세이처럼 읽혀야 하며, 하나의 비유 세계관을 " +
      "처음부터 끝까지 유지하고 서로 연결되어야 한다 — explanation의 마무리는 sajuStory에서 쓴 비유를 다시 " +
      "불러온다. 두 필드 모두 지정된 분량(사용자 메시지 참고)만큼 충분히 길게, 소제목은 항상 \"**소제목**\" " +
      "형태의 단독 줄로 써라."
    : "이 사주에 대한 감성적 해설(sajuStory)과 요약(summary)은 같은 사주로 이전에 이미 한 번 보여줬으므로 " +
      "이번엔 만들지 않는다 — candidates만 응답한다. 각 후보 explanation은 리포트 나열이 아니라 짧은 " +
      "에세이처럼 읽혀야 하며, 사용자 메시지에 지정된 분량만큼 충분히 길게, 소제목은 항상 \"**소제목**\" " +
      "형태의 단독 줄로 써라.";

  return (
    "너는 한국 전통 작명 서비스의 해설 작성자다. 사주 계산, 오행 판정, 용신 도출, 십신·공망 산출, 획수 계산은 " +
    "이미 코드가 끝냈다. 너는 그 결과만 받아 사용자가 이해하기 쉬운 자연어로 설명하고, 각 후보의 강점이 " +
    "고르게 부각되도록 서술하며(순위를 매기거나 우열을 비교하지 않는다), " +
    storyClause +
    " 각 후보의 strengths는 그 후보 한자의 고유한 뜻에 뿌리를 두어 다른 후보와 소재·표현이 겹치지 않게 쓴다" +
    "(실사용 빈도·인기는 언급하지 않는다). 신강/신약·용신·발음오행 상생·자원오행·수리사격 같은 전문 용어는 " +
    "그대로 쓰되, 처음 나올 때마다 짧은 쉬운 풀이나 비유를 곁들여 전문성과 이해하기 쉬움을 함께 잡는다. " +
...
export async function explainCandidates(input: ExplainCandidatesInput): Promise<NamingReport> {
  ...
  const hasStory = !input.isAdditionalRound;
  ...
  const response = await anthropic.messages.stream({
    ...
    max_tokens: 25000,
    system: buildSystemPrompt(hasStory),
    messages: [{ role: "user", content: buildUserPrompt(input, hasStory) }],
    output_config: {
      format: {
        type: "json_schema",
        schema: buildResponseSchema(hasStory),
      },
    },
  }).finalMessage();
